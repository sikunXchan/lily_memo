...
- コードフェンス、Markdown見出しは使わない（プレーンテキストが基本。要約やToDoは「・」の箇条書きでよい）
- 論文調の回答はしない
    ...
    if (activeNoteId !== undefined) {
      try {
        ...
        if (note) {
          ...
          const label = opts?.fullNote ? '本文全文' : '抜粋(先頭800文字)';
          noteContext = `\n\n# 現在ユーザーが開いているメモ\nタイトル: ${note.title || '無題'}\n${label}: ${body}`;
        }
      } catch { /* ignore */ }
    }
    ...
    try {
      ...
      if (searchReq) {
        ...
        const searchTurns: ChatTurn[] = [
          ...
          { role: 'user', text: `「${query}」を調べて、最初の質問に答えて。` },
        ];
      ...
      {mode === 'input' && (
        <div className="sikun-input-row" style={inputStyle}>
          <input
            ...
            disabled={loading}
          />
          <button className="sikun-send" onClick={() => void sendMessage()} disabled={!input.trim() || loading} aria-label="送信">
            <Send size={14} />
          </button>
        </div>
      )}
